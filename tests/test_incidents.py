...
async def test_update_incident(ac: AsyncClient):
    # Create incident
    create_response = await ac.post("/incidents/", json={
    ...
    # Update status
    response = await ac.patch(f"/incidents/{incident_id}", json={
...
async def test_update_nonexistent_incident(ac: AsyncClient):
    response = await ac.patch("/incidents/999999", json={
        "status": IncidentStatus.CLOSED
    })
